...
export function RegisterStudentPage() {
  ...
  const mutation = useMutation({
    mutationFn: (payload: FormValues) => authApi.registerStudent(payload),
    onSuccess: () => {
    ...
    <div className="min-h-screen px-6 py-12 md:px-12">
      <div className="mx-auto grid max-w-5xl gap-6 lg:grid-cols-[1.1fr_0.9fr]">
        <div className="rounded-3xl border border-white/70 bg-white/70 p-10 shadow-soft backdrop-blur animate-rise reveal-1">
          ...
          <h1 className="mt-4 text-4xl font-semibold text-ink">Student registration, simplified.</h1>
          <p className="mt-4 text-sm text-slate">
            Join your class with a teacher code. You will get access once approved.
          </p>
          <div className="mt-8 grid gap-4 md:grid-cols-2">
            <div className="rounded-2xl border border-white/70 bg-white/60 px-4 py-4">
              <p className="text-xs uppercase tracking-[0.3em] text-slate">Guided access</p>
              <p className="mt-2 text-sm text-ink">Your teacher verifies your account.</p>
            </div>
            <div className="rounded-2xl border border-white/70 bg-white/60 px-4 py-4">
              <p className="text-xs uppercase tracking-[0.3em] text-slate">Instant updates</p>
              <p className="mt-2 text-sm text-ink">Track approval status after signup.</p>
            </div>
          </div>
        </div>

        <div className="rounded-3xl border border-white/70 bg-white/80 p-8 shadow-soft backdrop-blur animate-rise reveal-2">
          <h2 className="text-2xl font-semibold text-ink">Student Registration</h2>
          <p className="mt-2 text-sm text-slate">Sign up with your assigned teacher code.</p>
          ...
          <form onSubmit={handleSubmit((data) => mutation.mutate(data))} className="mt-6 grid gap-4">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div>
                <label className="text-xs font-semibold uppercase tracking-[0.25em] text-slate">Roll No</label>
                <input {...register("rollNo")} className="mt-2 w-full rounded-2xl border border-white/70 bg-white/70 px-4 py-2" />
                {errors.rollNo && <p className="mt-1 text-xs text-rose-600">{errors.rollNo.message}</p>}
              </div>
              <div>
